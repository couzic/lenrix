...
const options = { logger: silentLoggerOptions }

describe('LenrixStore.loadFromFields$() loaders', () => {
   it('maps field stream', () => {
      const store = createStore({ name: 'name' }, options).loadFromFields$(
         ['name'],
         {
            greeting: map(({ name }) => 'Hello, ' + name)
         }
      )
...
const createRootStore = (
   loadFromName: (_: Observable<{ name: string }>) => Observable<'tata'>
) =>
...
describe('LenrixStore.loadFromFields$()', () => {
   ...
   let loadFromName: (_: Observable<{ name: string }>) => Observable<'tata'>
   beforeEach(() => {
      tata$ = new Subject()
   ...
   it('handles error', () => {
      const store = createStore(initialState, options).loadFromFields(
         ['name'],
         {
            toto: ({ name }) =>
...
it('loads from loading field', () => {
   ...
   const store = createStore({ pokemonName: 'pikachu' }, options)
